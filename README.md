...
- Non-Nix: place this checkout at `~/.config/nvim`. Install a C compiler, a current
  ...
  with pip/venv support before starting. After the initial startup, run
  `:TreesitterUpdateConfigured` and `:MasonUpdateConfigured` to install the configured
  parsers, language servers, formatters, ShellCheck and debugpy. Optional manual
  BasedPyright installation is `pip install basedpyright` (not `@anthropics/basedpyright`).
...
a .NET item. Open views follow scoped watchers and a throttled idle fallback; Git signs
also refresh on focus/write events. Pending MiniFiles edits defer directory refreshes.
Markdown actions use localleader.
...
`:checkhealth easy-dotnet`. Non-Nix users can rerun `:TreesitterUpdateConfigured` and
`:MasonUpdateConfigured` when updating or repairing parser and tool dependencies. For
non-Nix plugin updates run `:lua vim.pack.update()`, review the confirmation buffer,
`:w` to accept, then restart. Keep the generated `nvim-pack-lock.json`; do not edit
revisions by hand.
